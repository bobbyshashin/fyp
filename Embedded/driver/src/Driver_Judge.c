...
#define GET_BUFFER(x) JUDGE_DataBuffer[(uint8_t)(JUDGE_Data.nextDecodeOffset+(x))]
...
void JUDGE_Decode(uint32_t length) {
    ...
    if (JUDGE_Started) {
      ...
      JUDGE_RemainByte -= JUDGE_Data.nextDecodeOffset+1;

      repeatFlag = 1;
      while (JUDGE_RemainByte >= JUDGE_FRAME_HEADER_LENGTH && repeatFlag
          && VerifyCRC8(JUDGE_Data.nextDecodeOffset, JUDGE_FRAME_HEADER_LENGTH)) {
        repeatFlag = 0;
