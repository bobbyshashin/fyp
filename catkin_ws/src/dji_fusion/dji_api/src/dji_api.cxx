...
ros::Time frame_time;
DJIDrone* drone;
typedef bool (DJIDrone::*_dji_takeoff_func)();
typedef bool (DJIDrone::*_dji_landing_func)();
typedef bool (DJIDrone::*_dji_obtain_ctrl_func)();
typedef bool (DJIDrone::*_dji_release_ctrl_func)();
typedef bool (DJIDrone::*_dji_api_ctrl_data_func)(unsigned char ctrl_flag, float x, float y, float z, float yaw);


_dji_takeoff_func         takeoff_func;
_dji_landing_func 	      landing_func;
_dji_obtain_ctrl_func     obtain_ctrl_func;
_dji_release_ctrl_func    release_ctrl_func;
_dji_api_ctrl_data_func   api_ctrl_func;
//string frame_id;
...
void sdk_ctrl_callback(const geometry_msgs::Vector3& ctrl_velocity) {
    ...
    if(mission_status != INIT && mission_status != TAKEOFF) {
        ...
        **  z-axis: velocity   */
        ...
        (drone->*api_ctrl_func)(0x4b, ctrl_velocity.x, ctrl_velocity.y, ctrl_velocity.z, 0); // lock yaw to be zero
...
void sdk_cmd_callback(const std_msgs::UInt8& msg)
{
    ...
    case 3: // Release Control
        if( (drone->*release_ctrl_func)() )
        {
            ...
            cout<<"release control command has been send!"<<endl;
        }
        break;
    }
...
int main(int argc, char **argv)
{
    ...
    api_ctrl_sub = n.subscribe("sdk_ctrl", 1, sdk_ctrl_callback);
    api_cmd_sub  = n.subscribe("sdk_cmd",  1, sdk_cmd_callback);
